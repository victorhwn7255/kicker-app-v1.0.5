...
**2026-07-22 - plan B adopted (external pinger); "no plan B" reversed.** Two days on pure GitHub-schedule confirmed the throttle was worse than accepted: ~hourly effective runs with TWO ~6h overnight gaps, frequent concurrency-cancels (0 real failures), and volume dipping below the 30 floor (24 on 07-21) with burst-clustering (16/23 gaps <2min). Vic set up the free external pinger: cron-job.org POSTs the `workflow_dispatch` API every 15 min (fine-grained PAT `ticker-engine-pinger`, Actions:R+W on this repo only, in cron-job.org only; fail-notification on). Verified end-to-end (204 + a real dispatched run landed). Native `schedule` cron thinned `3,18,33,48` -> `8 * * * *` (hourly backup only, so the two triggers stop colliding; feed still degrades to hourly not zero if the pinger dies). Still $0/mo. This restores the even 15-min drip the 07-15 volume-tuning designed for.

**2026-07-20 - EC2 terminated; engine fully on GitHub Actions ("no plan B").** The 2-day parallel trial proved Actions runs work perfectly but GitHub throttles the SCHEDULE to ~hourly (occasional 2-4h gaps; off-peak cron minutes didn't help). Options weighed: a free external dispatcher (cron-job.org + PAT, restores 15-min rhythm) vs accepting the throttle. Vic chose **no plan B**: zero extra dependencies, accepting hourly-ish post clusters and ~30-40/day. Mitigation shipped: `ENGINE_MAX_BACKLOG_MIN=360` in the workflow (late slots publish late instead of being dropped by the 120-min stale guard - live timestamps keep late honest). Also that day: `MODEL_CALL_TIMEOUT_MS=180000` halved generation errors (23->10/day). Box terminated, `ssh ticker` config removed, ops docs rewritten. Ticker infra = $0/month. Plan B stays documented in 07-operations if steadier rhythm is ever wanted.

**2026-07-20 - floor widened to 30/day.** Two days of ~33-40 published (novelty gate eating rotation-heavy candidates) led Vic to widen the accepted published range to **30-60/day**; pipeline_health floor moved 40 -> 30. Same principle: gates stay strict, volume follows source freshness.

**2026-07-18 - published volume 40-60/day accepted as healthy.** Day two of cadence buckets: published fell to 41 (band 60-90) with novelty drops doubled - the `more` accounts (5-6 sources deep at best) hit rotation and the novelty gate correctly refused recycled takes. Vic's call: accept ~40-60 published/day as the healthy range rather than widen gates or soften multipliers. The 60-90 stays as the SLOT target; pipeline_health now flags published <40 (not <48) and ship rate <30% (not <45%). Path to more volume if ever wanted: deeper vault re-exports, never gate-loosening. Same day: the engine began migrating to GitHub Actions cron (`engine-tick.yml`) to take infra cost to $0 - see 07-operations.
