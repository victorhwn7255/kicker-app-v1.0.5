...
export function generatorSystem(): string {
  return [
    ...
    `5. LENGTH: between ${LENGTH.min} and ${LENGTH.max} characters - about 2 to 4 sentences.`,
    `   Aim for roughly 300-450; NEVER exceed ${LENGTH.max} - going over gets the post rejected`,
    '   outright, so STOP as soon as the key fact and its essential context are stated. Do NOT',
    '   pad, restate, or add filler; a tight post beats a padded one. Lead with the single most',
    '   important fact (given as KEY FACT). No hashtags, no emoji, no @-handles.',
    '',
...
export function verifierSystem(): string {
  return [
    ...
    '',
    'Calibration - the following are NOT violations (do not fail a post for them):',
    '- a specific value that falls INSIDE a range the source states ("about 75%" is',
    '  supported by a source saying "roughly 70-80%"); flag only values OUTSIDE the range.',
    '- first-person voice where the persona bio establishes the account IS the entity',
    '  the source describes ("we estimate our share..." from that company\'s own account).',
    '- rewording or reordering that keeps the same quantities, hedges, and meaning.',
    '',
